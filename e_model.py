...
class Aggregate(nn.Module):  # Enhanced MTN with ALL improvements
    def __init__(self, len_feature):
        super(Aggregate, self).__init__()
        self.len_feature = len_feature
        
        # Enhanced Pyramid Dilated Convolution Block
        ...
        self.transformer_block = TransformerEncoderBlock(int(len_feature/4), nhead=8, dim_feedforward=1024)
    ...
    def forward(self, x):
        ...
        pdc_out = self.pdc_block(out)  # Enhanced multi-scale convolutions: (B, F/4, T)
        se_out = self.se_module(pdc_out)  # Channel attention enhancement: (B, F/4, T)
        ...
        transformer_out = self.transformer_block(conv_out)  # Enhanced global attention: (B, F/4, T)
